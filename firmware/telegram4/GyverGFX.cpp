...
size_t GyverGFX::write(uint8_t data) {
  ...
  bool newPos = false;
  if (data == '\r') {
    _x = 0;  // получен возврат каретки
    newPos = true;
    data = 0;
  }
  if (data == '\n') {
    _y += _scaleY;  // получен перевод строки
    newPos = true;
    data = 0;
  }
  ...
  if (_println && data == ' ' && _x == 0) {
    data = 0;  // убираем первый пробел в строке
  }
      ...
      if (_scaleX == 1) {									// если масштаб 1
        ...
        for (uint8_t i = 0; i < _scaleX; i++)
          for (uint8_t j = 0; j < _scaleY; j++)
            dot(_x + i, _y + j, bitRead(newData, j));
        _x += _scaleX;
      }
    }
  }

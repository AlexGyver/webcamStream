...
if (!client->connected()) {
	...
	if (https.begin(*client, "https://api.telegram.org/"+command)) {  // HTTPS
		...
		if (httpCode > 0) {
			...
			//DEBUG("[HTTPS] GET... code: ");
			//DEBUGLN(httpCode);
			// file found at server?
			if (httpCode == HTTP_CODE_OK) {
				String payload = https.getString();
				DEBUGLN("[HTTPS] Received payload telegram: ");
				mess=payload;
				//DEBUGLN(String("1BTC = ") + payload + "USD");
			}
		} else {
			DEBUG("[HTTPS] GET telegram... failed, error: ");
			DEBUGLN(https.errorToString(httpCode).c_str());
		}

		https.end();
	} else {
		DEBUGLN("[HTTPS] Unable to connect BEAR::SSL telegram\n\r");
	}
...
void TelegramBOT::getUpdates(String offset)  {
	...
	if (mess!=""  && (WiFi.status() == WL_CONNECTED)) {
		DEBUG("Sent Update request messages up to : ");
		DEBUGLN(offset);
		String a="";
		int ch_count=0;
		String c;
	...
	if (mess=="") {     
		DEBUGLN("failed to update");
		return;
	}   
	if (i==0) {
		DEBUGLN("no new messages");
		DEBUGLN();
		message[0][0]="0";
	}
	else {
...
void TelegramBOT::sendMessage(String chat_id, String text, String reply_markup)  {
	...
	// if (sent==false) DEBUGLN("Message not delivered");
}
...
void TelegramBOT::analizeMessages(void)     {
	...
	for (int i=1; i<message[0][0].toInt()+1; i++)      {
		...
		//  for (int j=0; j<6; j++)	{
		//	DEBUGLN(message[i][j]);                                //print parsed data
		//  }
	}
}
